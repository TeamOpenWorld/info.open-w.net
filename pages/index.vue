<template>
    <div id="container">
        ...
        <section class="container py-5">
            <h1>お知らせ</h1>
            <p class="mb-4">TeamOpenWorldや関連サービス・団体に関するお知らせを掲載しています。</p>
            <div v-for="n in key_list" class="card my-2">
                <div class="card-header">
                    {{ obj_list[n]["tag"] }}
                </div>
                <div class="card-body">
                    <h5 class="card-title">{{ obj_list[n]["title"] }} <span class="badge badge-pill badge-dark">{{ obj_list[n]["date"] }}</span></h5>
                    <p class="card-text">{{ obj_list[n]["description"] }}</p>
                    <a :href="'/post/'+obj_list[n]['sourceBase'].slice( 0, -3 )" class="btn btn-primary">続きを読む</a>
                </div>
            </div>
        </section>
